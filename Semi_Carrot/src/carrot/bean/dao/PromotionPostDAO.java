...
public class PromotionPostDAO {
	...
	public void edit(PromotionPostDTO ppdto) throws Exception {
		Connection con = getConnection();
		String sql = "Update promotion_post SET post_title=?, post_content=?,post_price=?,post_phone=?,promotion_cate_num=? where post_no=?";
		PreparedStatement ps = con.prepareStatement(sql);
		
		ps.setString(1, ppdto.getPost_title());
		ps.setString(2, ppdto.getPost_content());
		ps.setLong(3, ppdto.getPost_price());
		ps.setString(4, ppdto.getPost_phone());
		ps.setLong(5, ppdto.getPromotion_cate_num());
