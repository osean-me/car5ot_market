...
public class UsedPostImgDAO {
	...
	public List<UsedPostImgDTO> getList(long post_no) throws Exception {
		Connection con = getConnection();
		String sql = "SELECT * FROM used_post_img WHERE post_no = ? ORDER BY post_img_no ASC";
		PreparedStatement ps = con.prepareStatement(sql);
	...
	// 회원 게시글 이미지 조회
	public UsedPostImgDTO getMember(long post_no) throws SQLException {
		Connection con = getConnection();
		String sql = "SELECT * FROM used_post_img WHERE post_no = ? ";
		PreparedStatement ps = con.prepareStatement(sql);
		ps.setLong(1, post_no);
		ResultSet rs = ps.executeQuery();
