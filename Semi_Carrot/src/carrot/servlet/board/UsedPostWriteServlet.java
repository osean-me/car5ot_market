...
public class UsedPostWriteServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			FileItem checkImg = map.get("used_post_img").get(0);
			boolean check = !map.get("used_cate_num").get(0).getString().isEmpty()
			...
			if (checkImg.getSize() > 0 && check) {
				...
				if(!check) {
					resp.sendRedirect(req.getContextPath() + "/board/used_write.jsp?content_error");
				} else {
					resp.sendRedirect(req.getContextPath() + "/board/used_write.jsp?img_error");
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
