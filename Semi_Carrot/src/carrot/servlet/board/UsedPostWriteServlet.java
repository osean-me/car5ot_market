 ...
 * 
 * MemberDTO mdto = (MemberDTO) req.getSession().getAttribute("memberinfo");
 * long member_no = mdto.getMember_no(); long member_addr_no =
 ...
 * updto.setMember_no(member_no); updto.setAddr_no(member_addr_no);
 * 
 * UsedPostDAO updao = new UsedPostDAO(); int post_no = updao.getSequence();
 * //번호 먼저 추출
 * 
 * updto.setPost_no(post_no); // 들어갈 번호 설정 후 updao.write(updto); // 등록
 * 
 * resp.sendRedirect("post_list.jsp"); // 일단은 리스트로 보내기 + 나중에 상세페이지 완성되면 완성 페이지로
 * 전송
 * 
 * 
